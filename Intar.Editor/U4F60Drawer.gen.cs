    ...
    public class U4F60Drawer : PropertyDrawer {
        ...
        internal static float Restore(ulong bits) {
            ...
            if (bits == ulong.MaxValue) {
                return (ulong.MaxValue + 1.0f) / U4F60.OneRepr;
            }

            var isNegative = bits < 0;
            var lowerBound = bits;
            var upperBound = bits + 1;
            var result = System.Math.Abs((float)(bits / U4F60.OneRepr));
            var precision = 0.1f;
            ...
            for (var digit = 0; digit < 10; digit++) {
                ...
                for (var num = 0; num < 10; num++) {
                    ...
                    if (next * U4F60.OneRepr >= upperBound) {
                        result = current;
                        break;
                    }
                    ...
                    // current * U4F60.OneRepr は upperBound 以下である.
                    // よって current * U4F60.OneRepr は整数型に安全にキャストできる.
                    // キャストした値は 0 方向に丸められるため,
                    // lowerBound「以上」という条件は厳密に成立する.
                    if ((uint)(current * U4F60.OneRepr) >= lowerBound) {
            ...
            return (bits + (Mathf.Sign(bits) * 0.5f)) / U4F60.OneRepr;
        }
