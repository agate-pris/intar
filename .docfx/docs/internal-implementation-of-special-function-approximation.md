...
$$
\begin{align*}
cos_{4\ 1}'\ \frac{\pi}{2} &= -1 &
cos_{4\ 2}'\ \frac{\pi}{3} &= -\frac{\sqrt{3}}{2} &
cos_{4\ 3}'\ \frac{\pi}{6} &= -\frac{1}{2}
\end{align*}
$$
...
$$
\begin{align*}
a_{4\ 1\ 2} &= \frac{ 8 -      \pi      }{    \pi^2} \approx 0.4922595829549115   \\
a_{4\ 1\ 4} &= \frac{16 -   4\ \pi      }{    \pi^4} \approx 0.035249578342151415 \\
a_{4\ 2\ 2} &= \frac{ 3^{7/2}\ \pi - 128}{ 4\ \pi^2} \approx 0.4791921675422863   \\
a_{4\ 2\ 4} &= \frac{ 3^{7/2}\ \pi - 144}{    \pi^4} \approx 0.02995355435513814  \\
a_{4\ 3\ 2} &= \frac{ 27     \ \pi -  16}{14\ \pi^2} \approx 0.498087713477496    \\
a_{4\ 3\ 4} &= \frac{ 54     \ \pi - 144}{ 7\ \pi^4} \approx 0.03761163067403257  \\
a_{4\ 4\ 2} &= \frac{ 49                }{10\ \pi^2} \approx 0.49647379984745516  \\
a_{4\ 4\ 4} &= \frac{ 18                }{ 5\ \pi^4} \approx 0.03695753611686361  \\
a_{4\ 5\ 2} &= \frac{160 - 3^{9/2}      }{ 4\ \pi^2} \approx 0.4991052271747134   \\
a_{4\ 5\ 4} &= \frac{144 - 3^{9/2}      }{    \pi^4} \approx 0.03802401344273002  \\
a_{4\ 6\ 2} &= \frac{ 24     \ \pi -  60}{    \pi^3} \approx 0.4966163414241372   \\
a_{4\ 6\ 4} &= \frac{ 80     \ \pi - 240}{    \pi^5} \approx 0.03701530604193427
\end{align*}
$$
...
以下は誤差の大きな関数を除いた誤差のグラフ｡

![cos_p4_error.svg](/images/cos_p4_error.svg)

$cos_{4\ 4}$ の正の最大の最大は $7.600704392574931 \times 10^-4$ ､ 負の最大の誤差は $-0.0010902926026138404$ ｡
$cos_{4\ 6}$ の正の最大の最大は $7.063890054643095 \times 10^-4$ ､ 負の最大の誤差は $-0.0011532615912570876$ ｡

$cos_{4\ 6}$ は誤差の平均値が 0 だが､ 誤差の最大値が小さいのは $cos_{4\ 4}$ なので使い分ける必要がある｡

Intar では定数項を以下のように変形する｡

$$
b_{4\ p\ m}=\lfloor \left(32768 \times \left(\frac{\pi}{2}\right)^m \right) a_{4\ p\ m}+0.5\rfloor
$$
